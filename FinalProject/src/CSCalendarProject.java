...
public class CSCalendarProject
{
	...
	public static void main(String[] args) {
		...
		while(option) {
			...
			UpdateCalendarMonth(grid,daysInMonth((month),year), firstDayOfMonth,day,description ,timeplace);	
			...
			if(yesno.equalsIgnoreCase("N")) {
				...
				while(optionSearch) {
					...
					//show calendar 
					String mySearch=SearchCalendar(grid,month,day,search,description,timeplace);
	...
	public static void UpdateCalendarMonth(String[][] gridUpdate,int monthDays ,int myFirstday,int day,String description,String timeplace) { 
		int count=0;
			...
			for (int j=0;j<gridUpdate[i].length; j++){										
				...
				if(row<=monthDays){
						...
						if( gridUpdate[i][j].equalsIgnoreCase(String.valueOf(day)) ){     
							gridUpdate[i][j]= day+ "*" + description + ":" + timeplace;
						}	
